...
- 006-film-crud: Full Film CRUD API (GET list/search/filter/pagination, GET by ID with flat detail, POST, PUT, DELETE hard-delete with inventory protection). Multi-table search across title/description/actor names via EF Core ILike. Category/rating/year-range filters. Lean list DTO (IDs only) vs rich detail DTO (flat actor/category/language names). MpaaRating enum with JsonStringEnumMemberName for PG-13/NC-17 serialization. 206 tests passing (70 validator unit + 43 integration + 93 existing).
